...
aiuser_endpoint_group = Group(name="aiuser_endpoint", description="Manage AIUser bot-wide endpoint settings.")


@aiuser_endpoint_group.command(name="endpoint")
...
async def endpoint_set_url(inter: discord.Interaction, url: Optional[str]):
    cog: Optional[red_commands.Cog] = inter.client.get_cog("AIUser")
    if not cog:
        if not inter.response.is_done():
            await inter.response.send_message("AIUser Cog not loaded!", ephemeral=True)
        else:
            await inter.followup.send("AIUser Cog not loaded!", ephemeral=True)
        return
